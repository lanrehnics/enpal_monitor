  ...
  Widget createWidgetUnderTest() {
    return MultiBlocProvider(
      ...
      child: const MaterialApp(
        home: Scaffold(
          body: SolarGenerationTab(),
        ),
      ),
    );
  }

  group('SolarGenerationTab Tests', () {
    ...
    testWidgets('shows line chart when solar data is loaded successfully', (tester) async {
      ...
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([SolarDataLoadedSuccessfully(MonitoringResponse(data: mockData))]),
    ...
    testWidgets('shows loading animation when solar data is loading', (tester) async {
      // Emit loading state
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([const SolarDataLoading()]),
        initialState: const MonitoringStateInitial(),
      );
    ...
    testWidgets('shows error snackbar when solar data fails to load', (tester) async {
      const errorMessage = 'Failed to load solar data';

      // Emit error state
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([SolarDataLoadedWithError(errorMessage)]),
        initialState: MonitoringStateInitial(),
      );
    ...
    testWidgets('calls fetchSolarMonitoringData on refresh', (WidgetTester tester) async {
      // Stub the fetchSolarMonitoringData method
      when(() => mockMonitoringCubit.fetchSolarMonitoringData(any(), force: true)).thenAnswer((_) async {});
      ...
      // Verify that the fetchSolarMonitoringData was called at least once
      verify(() => mockMonitoringCubit.fetchSolarMonitoringData(any(), force: true)).called(1);
    });
  });
}
