  ...
  Widget createWidgetUnderTest() {
    return MultiBlocProvider(
      ...
      child: const MaterialApp(
        home: Scaffold(
          body: BatteryConsumptionTab(),
        ),
      ),
    );
  }

  group('BatteryConsumptionTab Tests', () {
    ...
    testWidgets('shows line chart when battery consumption data is loaded successfully', (tester) async {
      ...
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([BatteryDataLoadedSuccessfully(MonitoringResponse(data: mockData))]),
    ...
    testWidgets('shows loading animation when battery consumption data is loading', (tester) async {
      // Emit loading state
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([const BatteryDataLoading()]),
        initialState: const MonitoringStateInitial(),
      );
    ...
    testWidgets('shows error snackbar when battery consumption data fails to load', (tester) async {
      const errorMessage = 'Failed to load battery consumption data';

      // Emit error state
      whenListen(
        mockMonitoringCubit,
        Stream.fromIterable([const BatteryDataLoadedWithError(errorMessage)]),
        initialState: const MonitoringStateInitial(),
      );
    ...
    testWidgets('calls fetchBatteryConsumptionData on refresh', (WidgetTester tester) async {
      // Stub the fetchBatteryConsumptionData method
      when(() => mockMonitoringCubit.fetchBatteryConsumptionData(any(), force: true)).thenAnswer((_) async {});
      ...
      // Verify that the fetchBatteryConsumptionData was called at least once
      verify(() => mockMonitoringCubit.fetchBatteryConsumptionData(any(), force: true)).called(1);
    });
  });
}
