...
/// A widget representing the Solar Generation tab.
///
/// Displays solar generation data in a chart and provides user controls
...
class SolarGenerationTab extends StatefulWidget {
  const SolarGenerationTab({super.key});

  @override
  State<StatefulWidget> createState() => _SolarGenerationState();
}

class _SolarGenerationState extends State<SolarGenerationTab> with MonitoringMixin, AutomaticKeepAliveClientMixin {
  @override
    ...
    return BlocListener<UtilityCubit, UtilityState>(
      listener: (_, state) {
        state.maybeWhen(
          ...
          fetchMonitoringData: () =>
              BlocProvider.of<MonitoringCubit>(context).fetchSolarMonitoringData(dateNotify.value),
          orElse: () {},
        );
      },
      child: RefreshIndicator(
        ...
        color: accentColour,
        onRefresh: () async {
          BlocProvider.of<MonitoringCubit>(context).fetchSolarMonitoringData(dateNotify.value, force: true);
        },
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              unitSwitch(),
              const SizedBox(height: 30),
              BlocConsumer<MonitoringCubit, MonitoringState>(
                listener: (_, state) {
                  if (state is SolarDataLoadedWithError) {
                    showErrorSnackBar(context, state.error);
                  }
                },
                builder: (_, state) {
                  return state.maybeWhen(
                    solarDataLoading: () => lineChartLoadingAnimation(),
                    solarDataLoadedSuccessfully: (res) => ElevatedContainer(
                      child: AspectRatio(
                        aspectRatio: 1.70,
